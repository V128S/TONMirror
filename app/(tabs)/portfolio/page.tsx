...
function StrategyRow({ strategy, pid }: { strategy: Strategy; pid: string }) {
  ...
  // synthetic spark — replace with real per-strategy P&L history once stored
  ...
  return (
    <div className="border border-phos-border-dim bg-bg-panel">
      {/* ps-like header row */}
      <div
        className="grid items-center px-2.5 py-2 border-b border-dashed border-phos-border-dim"
        style={{ gridTemplateColumns: "36px 1fr auto" }}
      >
        <span className="text-phos-mid text-[10px] tm-mono">{pid}</span>
        <Link href={`/leaders/${strategy.leaderWalletId}`}>
          <div className="text-phos-hi text-[11px] font-bold tm-mono">
            {strategy.leaderWallet.nickname}
          </div>
        ...
        <span
          className="tm-mono text-[9px] tracking-[0.1em] px-1.5 py-0.5 border"
          style={
            strategy.isPaused
              ? { borderColor: "#ffd500", color: "#ffd500" }
              : { borderColor: "#00ff66", color: "#c8ffd8" }
          }
        >
      ...
      <div className="px-2.5 py-2 flex items-center justify-between gap-2">
        <Sparkline
          ...
          width={170}
          height={22}
          color={strategy.isPaused ? "#4a8a5e" : "#00ffaa"}
          fill={false}
        />
...
export default function PortfolioPage() {
  ...
  // aggregate spark — fake but visually OK
  const aggSpark = Array.from({ length: 22 }).map(
    (_, i) => 30 + i * 3 + Math.sin(i * 0.8) * 6,
  );
      ...
      <div className="px-4 pt-2 space-y-3.5">
          ...
          <div className="flex justify-between">
            <div>
              ...
              {isLoading ? (
                ...
                <div className="tm-disp tm-glow text-[28px] text-phos-hi mt-0.5">
            ...
            <div className="text-right">
              ...
              <div className="text-[10px] text-phos-mid mt-0.5">
                {active.length} run · {paused.length} pause
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-1.5 mt-2.5">
            {[
              ...
              <div key={k.l} className="border-l-2 border-phos-border-dim pl-1.5">
                ...
                <div className={`tm-disp text-[14px] text-${k.c} mt-0.5`}>
        ...
        <div>
          ...
          <div className="mt-1.5">
            {/* ps header */}
            <div
              className="grid items-center px-2.5 py-1.5 border border-phos-border-dim border-b-0 bg-bg-el text-[8px] text-phos-mid tracking-[0.15em]"
              style={{ gridTemplateColumns: "36px 1fr auto" }}
            >
              <span>PID</span>
            ...
            {isLoading ? (
              ...
              <div className="space-y-0">
                {active.map((s, i) => (
                  <StrategyRow key={s.id} strategy={s} pid={`0x${(i + 1).toString().padStart(2, "0")}`} />
                ))}
              </div>
            ) : (
              <Card className="border-t-0">
                <p className="text-phos-mid text-[10px] text-center py-3 tm-mono">
                  ▢ NO · ACTIVE · PROCESSES ▢
                  <br />
                  <Link href="/leaders" className="text-phos-soft mt-1 inline-block">
                    browse leaders →
                  </Link>
                </p>
              </Card>
            ...
            <div className="mt-1.5 space-y-0">
              {paused.map((s, i) => (
                <StrategyRow
                  key={s.id}
                  strategy={s}
                  pid={`0x${(i + active.length + 1).toString().padStart(2, "0")}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="border border-phos-border-dim px-3 py-2.5">
          ...
          <div className="text-[9px] text-phos-mid mt-1.5 tm-mono">
            {formatUsd(totalVolume)} / $500 · resets 00:00 UTC
          </div>
        </div>
        ...
        {!isLoading && (strategies?.length ?? 0) === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-phos-mid text-[11px] mt-2 tm-mono">
              follow a leader wallet to start copy-trading.
            </p>
            <Link
              href="/leaders"
              className="inline-block mt-3 px-6 py-2.5 border border-phos text-phos-hi text-[12px] tracking-[0.2em] tm-mono"
