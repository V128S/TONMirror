...
export default function HomePage() {
  ...
  // Map activity to LiveFeed shape
      ...
      <div className="px-4 pt-2 space-y-3.5">
        ...
        <div>
          ...
          <CornerBox className="mt-2 border border-phos-border bg-bg-panel px-3 py-3.5">
            ...
            <div className="mt-3 pt-2.5 flex justify-between items-center text-[9px] text-phos-mid border-t border-dashed border-phos-border-dim">
              ...
              <span>
                last sync: <span className="tm-blink text-phos">NOW</span>
              </span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            <div
              key={c.k}
              className="border border-phos-border-dim p-2"
        ...
        <div>
          ...
          <div className="mt-1.5 border border-phos-border-dim bg-bg-panel px-2.5 py-1.5">
            {actLoading || feed.length === 0 ? (
              <div className="py-6 text-center text-[10px] text-phos-mid">
                <BlinkCaret /> awaiting signal…
              </div>
            ) : (
              ...
              {activity.slice(0, 3).map((event) => (
                <div key={event.id} className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <span className="text-[9px] text-phos-mid">
                      {formatRelativeTime(event.timestamp)}
                    </span>
                  </div>
                </div>
              ))}
              <Link href="/activity" className="inline-flex text-phos-soft text-[10px] mt-1">
                View all activity →
              </Link>
            </CardBody>
          </Card>
